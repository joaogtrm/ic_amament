...
Em relação a mãe:
  ...
  2) Em relação a gravidez: é o primeiro parto(paridade), se já amamentou anteriormente (e quando começou a introdução alimentar), se a gravidez foi planejada, se a gravidez foi desejada, quantos dias a mãe ficou internada no pós-parto, se houve complicações no pós-parto, se a mãe sente dor ao amamentar, tipo de parto (cesárea ou vaginal), idade gestacional do parto, se teve ajuda durante a gravidez e no pós-parto, se houve dificuldade para amamentar, se a amamentação ainda era exclusiva (sem introdução alimentar) no tempo 3, 

características do recém-nascido :
  1) Gerais: sexo, peso logo após o parto, média do ganho de peso diário. 
  2)Em relação a obstetrícia:se o recém-nascido precisou ir para a uti, quanto tempo o recém-nascido ficou na uti, se houve orientação prévia em relação à amamentação, se já retornou ao trabalho, tempo em que houve começou a introdução alimentar e se existe censura nesse tempo.

# Análise descritiva

A base conta com 254 observações e 30 variáveis, com 538 dados faltantes. Na base de dados existem variáveis relacionadas ao recém-nascido, as mães, e também às relacionadas ao período da gravidez (variáveis neonatais). Olhando cada grupo de variáveis de forma separada:
...
No caso das variáveis numéricas, elas foram separadas em grupos, para assim ser possível de comparas as curvas de sobrevivência. As variáveis numéricas consideradas, e seus novos valores, se encontram abaixo:   
```{r}
par(mfrow=c(2,2))
plot_bar <- list()
i <- "idade_menor26"
...
i <- "dias_int_mae_pos_parto_menor_igual3"
dat <- df
fil <- "censura"
...
plot_geral
```
Note que existe um valor P em cada gráfico, ele representa o P-valor do teste de log-rank, um teste de hipótese em que a hipótese nula é que as curvas de sobrevivência de cada grupo são iguais, e a hipótese alternativa é que existe diferença entre as curvas de sobrevivência dos grupos. Para um nível de confiança de 95%, um P-valor de 0.05 é adotado, ou seja, quando o P-valor for menor ou igual a 0.05, significa que as curvas de sobrevivência de cada grupo são diferentes, apontando que aquela variável impacta no tempo em que começa a introdução alimentar.
...
De acordo com teste de log-rank, podemos afirmar que os seguintes fatores são agravantes para as mães iniciarem a introdução alimentar precocemente:
  ...
  7)Ter orientação em relação a amamentação antes do parto.
